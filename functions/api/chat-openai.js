// Cloudflare Pages Function for OpenAI Chat
export async function onRequestPost(context) {
    try {
        const { messages, model = 'gpt-4-turbo-preview' } = await context.request.json();
        ...
        if (!apiKey) {
            return new Response(JSON.stringify({ error: 'Brak klucza API OpenAI' }), {
                status: 500,
                headers: { 'Content-Type': 'application/json' }
            });
        }
...
Obszary ekspertyzy: Strategia biznesowa, Marketing, Finanse, Prawo gospodarcze, Zarządzanie, CRM, E-commerce, Produktywność`
        ...
        if (!response.ok) {
            ...
            return new Response(JSON.stringify({ error: 'Błąd OpenAI API', details: errorData.error?.message }), {
