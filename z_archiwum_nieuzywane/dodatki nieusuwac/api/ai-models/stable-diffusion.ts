  ...
  return {
    ...
    sections: {
      ...
      'Funkcje': {
        ...
        style_presets: 'Gotowe style: photographic, digital-art, comic-book, fantasy-art, line-art, analog-film, neon-punk, isometric',
    ...
    examples: {
      'Podstawowy': '{"prompt": "beautiful landscape with mountains", "model": "stable-diffusion-xl"}',
      'Z negative': '{"prompt": "portrait of a cat", "negative_prompt": "blurry, low quality", "steps": 30}',
      'Stylizowany': '{"prompt": "cyberpunk city", "style_preset": "neon-punk", "guidance_scale": 15}',
      'Wysokiej jakości': '{"prompt": "detailed artwork", "model": "stable-diffusion-xl", "steps": 50, "samples": 2}'
    },
    best_practices: {
...
async function generateStableDiffusionImage(options: any) {
  ...
  return {
    imageUrl: `https://stable-diffusion.mybonzo.com/generate/${width}x${height}?prompt=${encodeURIComponent(prompt)}&model=${model}&seed=${seed}`,
    metadata: {
      prompt: prompt,
      negative_prompt: negative_prompt,
      model: model,
    ...
    variations: [
      `https://stable-diffusion.mybonzo.com/variant1/${width}x${height}?seed=${seed+100}`,
      `https://stable-diffusion.mybonzo.com/variant2/${width}x${height}?seed=${seed+200}`,
      `https://stable-diffusion.mybonzo.com/variant3/${width}x${height}?seed=${seed+300}`
    ],
