...
// Definicje komend głosowych dla różnych języków
const voiceCommands: Record<string, CommandPattern[]> = {
  'pl-PL': [
    // POLACZEK Commands
    {
      patterns: [
...
// Funkcja rozpoznawania komend
function recognizeCommand(command: string, language: string): VoiceCommandResponse {
  ...
  return {
    ...
    execution: {
      executed: false,
      message: language === 'pl-PL' ? 
        'Nie rozpoznano komendy. Spróbuj ponownie lub sprawdź dostępne komendy.' :
        'Command not recognized. Try again or check available commands.'
    },
    suggestions: generateSuggestions('help', language)
  };
}

// Funkcja wykonywania komend
function executeCommand(action: string, parameters?: Record<string, any>): { executed: boolean; result?: any; message: string } {
  switch (action) {
    ...
    if (confidence < 60) {
      return new Response(JSON.stringify({
        ...
        execution: {
          executed: false,
          message: language === 'pl-PL' ? 
            'Komenda nie jest wystarczająco wyraźna. Spróbuj ponownie.' :
            'Command not clear enough. Please try again.'
        },
        suggestions: generateSuggestions('help', language)
      }), {
        status: 200,
