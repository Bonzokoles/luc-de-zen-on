...
- other: ogólne pytania, feedback, pozostałe`;
    ...
    // Here you would typically save to database and trigger workflows
    // For now, we'll just return the classification
...
export const GET: APIRoute = async ({ url }) => {
  ...
  return new Response(JSON.stringify({
    ...
    updates: [
      { timestamp: new Date().toISOString(), message: 'Ticket został utworzony i przypisany' },
      { timestamp: new Date(Date.now() - 3600000).toISOString(), message: 'Zespół rozpoczął analizę' }
    ]
  }), {
    status: 200,
    headers: { 'Content-Type': 'application/json' }
