...
const artMediaTypes = [
    "Photo, DSLR", "Analog Photo", "Lomography Photo", "Pinhole Photo", "BW Photo", "Wet-Plate Photo",
    "Painting", "Oil Painting", "Watercolor", "Acrylic", "Gouache", "Tempera", "Fresco", "Encaustic",
    "Drawing", "Pencil", "Charcoal", "Pastel", "Ink", "Pen and Ink", "Conte Crayon", "Graphite",
    "Print", "Lithography", "Woodcut", "Engraving", "Etching", "Screen Print", "Linoleum Print",
    "Digital Art", "Concept Art", "Matte Painting", "3D Render", "Pixel Art", "Vector Art",
    "Mixed Media", "Collage", "Mosaic", "Stained Glass", "Tapestry", "Embroidery",
    "Sculpture", "Bronze", "Marble", "Clay", "Wood Carving", "Stone Carving", "Metal Work",
    "Street Art", "Graffiti", "Mural", "Stencil Art", "Spray Paint"
];

// Quality enhancers and technical terms
const qualityEnhancers = [
    "masterpiece", "best quality", "ultra detailed", "8k resolution", "highly detailed",
    "photorealistic", "hyperrealistic", "professional", "award winning", "trending on artstation",
    "sharp focus", "studio lighting", "vivid colors", "dramatic lighting", "cinematic lighting",
    "volumetric lighting", "soft lighting", "natural lighting", "golden hour", "blue hour",
    "depth of field", "bokeh", "rule of thirds", "symmetrical composition", "dynamic pose",
    "intricate details", "fine art", "gallery quality", "museum piece", "artistic"
];

// Negative prompt elements to avoid
const negativePrompts = [
    "blurry", "low quality", "pixelated", "jpeg artifacts", "watermark", "signature",
    "text", "logo", "username", "worst quality", "low res", "normal quality",
    "amateur", "poor composition", "bad anatomy", "deformed", "mutated", "extra limbs",
    "missing limbs", "floating limbs", "disconnected limbs", "malformed hands", "poor hands",
    "mutated hands", "ugly", "disgusting", "gross", "disturbing", "nsfw"
];
...
function enhancePrompt(
    ...
    if (options.artistStyle) {
        const artist = artistStyles.find(a =>
    ...
    // Add color palette
    if (options.colorPalette && colorPalettes[options.colorPalette as keyof typeof colorPalettes]) {
    ...
    if (options.mood) {
        ...
        if (moodDescriptors[options.mood as keyof typeof moodDescriptors]) {
            enhancedPrompt = `${enhancedPrompt}, ${moodDescriptors[options.mood as keyof typeof moodDescriptors]}`;
    ...
    if (options.enhanceCreativity) {
        const creativeTerms = [
            "creative composition", "unique perspective", "artistic interpretation",
            "innovative design", "imaginative", "original"
        ];
