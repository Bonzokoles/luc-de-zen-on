...
export class MySecretsAPI {
  ...
  async getHuggingFaceKey(): Promise<string | null> {
    return (
      (await this.getSecret("HUGGINGFACE_API_KEY")) ||
  ...
  async getGoogleAIKey(): Promise<string | null> {
    return (
      (await this.getSecret("GOOGLE_AI_STUDIO_API_KEY")) ||
   ...
   */
  async validateProviderKeys(): Promise<
    { provider: string; hasKey: boolean; source?: string }[]
  > {
    const results = [];
   ...
   */
  async debugSecretSources(): Promise<
    { source: string; available: boolean; keys?: string[] }[]
  > {
    const results = [];
    ...
    for (const source of this.sources) {
      ...
      try {
        ...
        if (
          source.name === "CloudflareSecretsStore" &&
          this.env.SECRETS_STORE?.list
        ) {
          try {
 ...
 */
export function createMySecretsAPI(
  env: any,
  config?: Partial<MySecretsConfig>
): MySecretsAPI {
...
export async function getProviderAuthHeader(
  env: any,
  provider: string
): Promise<string | null> {
  const secretsAPI = createMySecretsAPI(env);
