...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (PUBLIC_PATHS.some(publicPath => path.startsWith(publicPath)) || !path.startsWith('/api/')) {
    return next();
  }
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return new Response(JSON.stringify({ error: "Brak autoryzacji: Nieprawidłowy nagłówek." }), { status: 401 });
  }
  ...
  try {
    ...
    if (!payload.userId || !payload.clientId) {
      throw new Error('Token nie zawiera wymaganych danych (userId, clientId).');
    }
    ...
    const errorMessage = error instanceof Error ? error.message : "Nieznany błąd tokena.";
    return new Response(JSON.stringify({ error: `Błąd autoryzacji: ${errorMessage}` }), { status: 401 });
  }
});
