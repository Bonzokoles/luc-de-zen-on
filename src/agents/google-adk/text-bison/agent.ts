...
export class TextBisonAgent extends BaseGoogleADKAgent {
  ...
  constructor(config: AgentConfig) {
    ...
    this.supportedLanguages = [
      'pl', 'en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 
      'zh', 'ar', 'hi', 'tr', 'nl', 'sv', 'da', 'no', 'fi', 'cs', 
      'sk', 'hu', 'ro', 'bg', 'hr', 'sl', 'et', 'lv', 'lt'
    ];
  ...
  private determineRequestType(message: string): 'generate' | 'analyze' | 'summarize' | 'translate' | 'improve' | 'rewrite' | 'general' {
    const lowerMessage = message.toLowerCase();
    
    if (lowerMessage.includes('napisz') || lowerMessage.includes('wygeneruj tekst') || 
        lowerMessage.includes('stwórz artykuł') || lowerMessage.includes('generate text')) {
      return 'generate';
    }
    
    if (lowerMessage.includes('przeanalizuj tekst') || lowerMessage.includes('analiza tekstu') ||
        lowerMessage.includes('analyze text') || lowerMessage.includes('sentiment')) {
      return 'analyze';
    }
    
    if (lowerMessage.includes('podsumuj') || lowerMessage.includes('streszczenie') ||
        lowerMessage.includes('summarize') || lowerMessage.includes('skróć tekst')) {
      return 'summarize';
    }
    ...
    if (lowerMessage.includes('popraw tekst') || lowerMessage.includes('improve') ||
        lowerMessage.includes('korekta') || lowerMessage.includes('gramatyka')) {
      return 'improve';
    }
    
    if (lowerMessage.includes('przepisz') || lowerMessage.includes('rewrite') ||
        lowerMessage.includes('parafrazuj') || lowerMessage.includes('rephrase')) {
      return 'rewrite';
    }
  ...
  private async generateText(prompt: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd generowania tekstu: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async analyzeText(message: string): Promise<TextAnalysisResult> {
    try {
      ...
      throw new Error(`Błąd analizy tekstu: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async summarizeText(message: string): Promise<any> {
    try {
      ...
      return {
        ...
        compressionRatio: data.summary ? (data.summary.length / textToSummarize.length * 100).toFixed(1) : 0
      };

    } catch (error) {
      throw new Error(`Błąd podsumowania tekstu: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async translateText(message: string): Promise<any> {
    try {
      ...
      const data = await response.json();
      return {
        translatedText: data.translatedText || 'Nie udało się przetłumaczyć tekstu',
      ...
      throw new Error(`Błąd tłumaczenia tekstu: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async improveText(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd poprawy tekstu: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async rewriteText(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd przepisania tekstu: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private extractTextFromMessage(message: string): string | null {
    ...
    const cleaned = message
      .replace(/^(napisz|przeanalizuj|podsumuj|przetłumacz|popraw|przepisz)\s*/i, '')
  ...
  private extractStyleFromPrompt(prompt: string): 'formal' | 'casual' | 'creative' | 'technical' | 'academic' {
    const lowerPrompt = prompt.toLowerCase();
    
    if (lowerPrompt.includes('formal') || lowerPrompt.includes('oficjalny')) return 'formal';
    if (lowerPrompt.includes('casual') || lowerPrompt.includes('nieformalny')) return 'casual';
    if (lowerPrompt.includes('creative') || lowerPrompt.includes('kreatywny')) return 'creative';
    if (lowerPrompt.includes('technical') || lowerPrompt.includes('techniczny')) return 'technical';
    if (lowerPrompt.includes('academic') || lowerPrompt.includes('akademicki')) return 'academic';
    
    return 'casual';
  }

  private extractFormatFromPrompt(prompt: string): 'paragraph' | 'bullet_points' | 'essay' | 'summary' | 'article' {
    const lowerPrompt = prompt.toLowerCase();
    
    if (lowerPrompt.includes('bullet') || lowerPrompt.includes('lista')) return 'bullet_points';
    if (lowerPrompt.includes('essay') || lowerPrompt.includes('esej')) return 'essay';
    if (lowerPrompt.includes('summary') || lowerPrompt.includes('streszczenie')) return 'summary';
    if (lowerPrompt.includes('article') || lowerPrompt.includes('artykuł')) return 'article';
  ...
  private extractTargetLanguage(message: string): string {
    ...
    if (lowerMessage.includes('na polski') || lowerMessage.includes('po polsku')) return 'pl';
    if (lowerMessage.includes('na angielski') || lowerMessage.includes('po angielsku')) return 'en';
    if (lowerMessage.includes('na niemiecki') || lowerMessage.includes('po niemiecku')) return 'de';
    if (lowerMessage.includes('na francuski') || lowerMessage.includes('po francusku')) return 'fr';
    if (lowerMessage.includes('na hiszpański') || lowerMessage.includes('po hiszpańsku')) return 'es';
    if (lowerMessage.includes('na włoski') || lowerMessage.includes('po włosku')) return 'it';
    if (lowerMessage.includes('na rosyjski') || lowerMessage.includes('po rosyjsku')) return 'ru';
...
export function createTextBisonAgent(config?: Partial<AgentConfig>): TextBisonAgent {
  const defaultConfig: AgentConfig = {
    ...
    color: '#1976d2',
    priority: 'HIGH',
    description: 'Advanced text processing AI for generation, analysis, and language tasks'
  };
