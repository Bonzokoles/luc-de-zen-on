...
export class VertexAIAgent extends BaseGoogleADKAgent {
  ...
  constructor(config: AgentConfig) {
    ...
    if (endpointBase) {
      if (!/^https?:\/\//.test(endpointBase)) {
        throw new Error(
          "VERTEX_AI_API_BASE (absolute URL) must be configured for Vertex AI requests"
        );
      }
  ...
  private determineRequestType(
    ...
    if (
      lowerMessage.includes("przewiduj") ||
      lowerMessage.includes("predict") ||
      lowerMessage.includes("inference") ||
      lowerMessage.includes("model prediction")
    ) {
      return "predict";
    }
    ...
    if (
      ...
      lowerMessage.includes("konwersacja") ||
    ...
    if (
      lowerMessage.includes("generuj kod") ||
      lowerMessage.includes("generate code") ||
      lowerMessage.includes("napisz kod") ||
      lowerMessage.includes("stwórz funkcję")
    ) {
      return "generate_code";
    }

    if (
      lowerMessage.includes("analizuj obraz") ||
      lowerMessage.includes("analyze image") ||
      lowerMessage.includes("rozpoznaj") ||
    ...
    if (
      lowerMessage.includes("wdróż model") ||
      lowerMessage.includes("deploy model") ||
      lowerMessage.includes("deployment") ||
      lowerMessage.includes("uruchom model")
    ) {
      return "deploy_model";
    }

    if (
      lowerMessage.includes("lista modeli") ||
      lowerMessage.includes("list models") ||
      lowerMessage.includes("dostępne modele") ||
      lowerMessage.includes("available models")
    ) {
      return "list_models";
    }
    ...
    if (
      ...
      lowerMessage.includes("wsadowo") ||
      lowerMessage.includes("masowo") ||
      lowerMessage.includes("wiele predykcji")
    ) {
  ...
  private extractPromptFromMessage(message: string): string {
    ...
    return message
      ...
      .replace(
        /^(przewiduj|predict|chat|generuj|analyze|wdróż|deploy|lista|list)\s*/i,
        ""
      )
      .trim();
  }
  ...
  private extractBatchDataFromMessage(
    message: string
    ...
    if (jsonSnippet) {
      try {
        ...
        throw new Error(
          "Batch prediction payload could not be parsed as JSON."
        );
      }
    }
    throw new Error(
      "Batch prediction payload not provided. Please include a JSON array of instances."
    );
  }
...
export function createVertexAIAgent(
