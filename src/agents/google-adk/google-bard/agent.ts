...
export class GoogleBardAgent extends BaseGoogleADKAgent {
  ...
  private determineRequestType(message: string): 'creative_writing' | 'conversation' | 'research' | 'brainstorm' | 'explain' | 'compare' | 'plan' | 'general' {
    const lowerMessage = message.toLowerCase();
    
    if (lowerMessage.includes('napisz') || lowerMessage.includes('stwórz') || 
        lowerMessage.includes('opowiadanie') || lowerMessage.includes('wiersz') ||
        lowerMessage.includes('write') || lowerMessage.includes('create story') ||
        lowerMessage.includes('poem') || lowerMessage.includes('essay')) {
      return 'creative_writing';
    }
    
    if (lowerMessage.includes('rozmowa') || lowerMessage.includes('kontynuuj') ||
        lowerMessage.includes('conversation') || lowerMessage.includes('chat')) {
      return 'conversation';
    }
    
    if (lowerMessage.includes('zbadaj') || lowerMessage.includes('research') ||
        lowerMessage.includes('znajdź informacje') || lowerMessage.includes('investigate')) {
      return 'research';
    }
    
    if (lowerMessage.includes('burza mózgów') || lowerMessage.includes('brainstorm') ||
        lowerMessage.includes('pomysły') || lowerMessage.includes('ideas')) {
      return 'brainstorm';
    }
    
    if (lowerMessage.includes('wyjaśnij') || lowerMessage.includes('explain') ||
        lowerMessage.includes('co to znaczy') || lowerMessage.includes('what does')) {
      return 'explain';
    }
    
    if (lowerMessage.includes('porównaj') || lowerMessage.includes('compare') ||
        lowerMessage.includes('różnice') || lowerMessage.includes('vs') ||
        lowerMessage.includes('differences')) {
      return 'compare';
    }
    
    if (lowerMessage.includes('plan') || lowerMessage.includes('zaplanuj') ||
        lowerMessage.includes('strategia') || lowerMessage.includes('schedule')) {
      return 'plan';
    }
  ...
  private async generateCreativeContent(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd tworzenia treści kreatywnej: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async continueConversation(message: string): Promise<BardResponse> {
    try {
      ...
      throw new Error(`Błąd rozmowy z Bard: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async performResearch(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd badania: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async brainstormIdeas(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd burzy mózgów: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async explainConcept(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd wyjaśniania: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async compareOptions(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd porównania: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async createPlan(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd planowania: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private async handleGeneralQuery(message: string): Promise<any> {
    try {
      ...
      throw new Error(`Błąd zapytania Bard: ${error instanceof Error ? error.message : 'Nieznany błąd'}`);
    }
  }
  ...
  private parseCreativeTask(message: string): CreativeTask {
    ...
    if (lowerMessage.includes('opowiadanie') || lowerMessage.includes('story')) type = 'story';
    if (lowerMessage.includes('wiersz') || lowerMessage.includes('poem')) type = 'poem';
    if (lowerMessage.includes('esej') || lowerMessage.includes('essay')) type = 'essay';
    if (lowerMessage.includes('scenariusz') || lowerMessage.includes('script')) type = 'script';
    if (lowerMessage.includes('piosenka') || lowerMessage.includes('song')) type = 'song';
  ...
  private extractLength(message: string): CreativeTask['length'] {
    ...
    if (lowerMessage.includes('krótki') || lowerMessage.includes('short')) return 'short';
    if (lowerMessage.includes('długi') || lowerMessage.includes('long')) return 'long';
  ...
  private extractTone(message: string): CreativeTask['tone'] {
    ...
    if (lowerMessage.includes('humorystyczny') || lowerMessage.includes('funny')) return 'humorous';
  ...
  private extractConceptToExplain(message: string): string {
    return message
      .replace(/^(wyjaśnij|explain|co to znaczy)\s*/i, '')
      .trim();
  }

  private extractOptionsToCompare(message: string): string[] {
    const text = message
      .replace(/^(porównaj|compare)\s*/i, '')
      .trim();
  ...
  private extractPlanRequest(message: string): { goal: string; timeframe: string; complexity: string } {
    const goal = message
      .replace(/^(plan|zaplanuj|strategia)\s*/i, '')
      .trim();
  ...
  private extractTimeframe(message: string): string {
    ...
    if (lowerMessage.includes('dzień') || lowerMessage.includes('day')) return 'daily';
    if (lowerMessage.includes('tydzień') || lowerMessage.includes('week')) return 'weekly';
    if (lowerMessage.includes('miesiąc') || lowerMessage.includes('month')) return 'monthly';
    if (lowerMessage.includes('rok') || lowerMessage.includes('year')) return 'yearly';
  ...
  private extractComplexity(message: string): string {
    ...
    if (lowerMessage.includes('prosty') || lowerMessage.includes('simple')) return 'simple';
    if (lowerMessage.includes('skomplikowany') || lowerMessage.includes('complex')) return 'complex';
  ...
  private formatComparisonResult(result: any): string {
    ...
    if (result.comparison_table && Object.keys(result.comparison_table).length > 0) {
    ...
    try {
      ...
      return {
        ...
        data: response.ok ? { 
          ...
          features: ['creative_writing', 'conversation', 'research', 'brainstorming']
        } : null,
...
export function createGoogleBardAgent(config?: Partial<AgentConfig>): GoogleBardAgent {
  const defaultConfig: AgentConfig = {
    ...
    color: '#ea4335',
    priority: 'HIGH',
    description: 'Google Bard AI for creative writing, conversation, and research'
  };
