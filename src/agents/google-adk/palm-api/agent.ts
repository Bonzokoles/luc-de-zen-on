...
export class PaLMAPIAgent extends BaseGoogleADKAgent {
  ...
  constructor(config: AgentConfig) {
    super(config);
    this.palmEndpoint =
      (config as any).palmEndpoint ?? process.env.PALM_API_ENDPOINT ?? "";
  ...
  private determineRequestType(
    ...
    if (
      ...
      lowerMessage.includes("porozmawiaj") ||
      lowerMessage.includes("odpowiedz")
    ) {
      return "chat";
    }
    ...
    if (
      ...
      lowerMessage.includes("wektor") ||
      lowerMessage.includes("reprezentacja") ||
      lowerMessage.includes("podobieństwo")
    ) {
      return "embedding";
    }

    if (
      lowerMessage.includes("dokończ") ||
      lowerMessage.includes("complete") ||
      lowerMessage.includes("kontynuuj tekst") ||
    ...
    try {
      ...
      if (response.candidates && response.candidates.length > 0) {
        return {
          ...
          language: language,
          explanation: this.extractExplanationFromCode(
  ...
  private async generateEmbedding(message: string): Promise<any> {
    try {
      ...
      throw new Error(
        `Błąd generowania embeddingu PaLM: ${
          error instanceof Error ? error.message : "Nieznany błąd"
        }`
      );
    }
    ...
    try {
      ...
      if (response.candidates && response.candidates.length > 0) {
        return {
          ...
          compression_ratio: (
            ((textToSummarize.length - response.candidates[0].output.length) /
  ...
  private extractTextForEmbedding(message: string): string {
    return message.replace(/^(embedding|wektor|reprezentacja)\s*/i, "").trim();
  }
  ...
  private extractTextToSummarize(message: string): string {
    return message.replace(/^(podsumuj|summarize|streszczenie)\s*/i, "").trim();
  }

  private extractTextToTranslate(message: string): string {
    const match =
      message.match(/przetłumacz\s+"([^"]+)"/i) ||
  ...
  private extractTargetLanguage(message: string): string {
    ...
    if (
      ...
      lowerMessage.includes("to polish")
    )
      return "Polish";
    if (
    ...
    if (
      ...
      lowerMessage.includes("to german")
    )
      return "German";
    if (
      lowerMessage.includes("na francuski") ||
      lowerMessage.includes("to french")
    )
      return "French";
    if (
      lowerMessage.includes("na hiszpański") ||
      lowerMessage.includes("to spanish")
    )
      return "Spanish";
    if (
      lowerMessage.includes("na włoski") ||
      lowerMessage.includes("to italian")
    )
      return "Italian";
  ...
  private formatCodeResult(result: any): string {
    ...
    if (
      result.explanation &&
      result.explanation !== "Brak wyjaśnienia w kodzie"
    ) {
  ...
  private formatEmbeddingResult(result: any): string {
    ...
    output += `📄 **Tekst:** ${result.text.substring(0, 100)}${
      result.text.length > 100 ? "..." : ""
    }\n`;
    output += `🤖 **Model:** ${result.model_used}\n\n`;

    if (result.embedding.length > 0) {
      output += `🔢 **Wektor (pierwsze 5 wartości):** [${result.embedding
        .slice(0, 5)
...
export function createPaLMAPIAgent(
  ...
  const defaultConfig: AgentConfig = {
    ...
    priority: "HIGH",
    description:
      "Google PaLM API for advanced language processing and generation",
  };
