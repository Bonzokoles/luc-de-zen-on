...
const VoiceAssistantPolaczek: React.FC<VoiceAssistantPolaczekProps> = ({
  ...
  const handleUserInput = async (input: string) => {
    ...
    try {
      ...
      const errorResponse =
        "Przepraszam, mam problem z połączeniem. Spróbuj ponownie.";
      setConversationHistory((prev) => [
        ...prev,
        { type: "assistant", text: errorResponse },
      ]);
  ...
  const toggleAssistant = () => {
    if (!isActive) {
      ...
      speakText(
        "Cześć! Jestem POLACZEK. Pytaj mnie o system BONZO! Po mojej odpowiedzi automatycznie zacznę słuchać twojego następnego pytania."
      );
    } else {
      setIsActive(false);
