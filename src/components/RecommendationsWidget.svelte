<script lang="ts">
  ...
  async function fetchRecommendations() {
    ...
    try {
      const historyArray = history.trim()
...
>
  <div class="mb-6">
    ...
    <p class="text-gray-400">
      System rekomendacji produktów i usług oparty na preferencjach użytkownika
    </p>
  </div>
    ...
    {#if loading}
      <div class="text-center py-8">
        ...
        <p class="text-cyan-300 uppercase tracking-wide">
          Analizuję preferencje i generuję rekomendacje...
        </p>
      </div>
    {/if}
      ...
      <div class="mt-6">
        ...
        <div class="grid gap-4">
          {#each recommendations as rec, idx}
            <div
              class="p-4 rounded-lg border {getPriorityColor(
                rec.priority || 3
              )} transition-all duration-300 hover:shadow-xl hover:border-opacity-80 bg-gray-900/20"
            >
              <div class="flex justify-between items-start mb-3">
                <h4
                  class="text-white font-bold text-lg uppercase tracking-wide"
                >
                  {rec.title}
                </h4>
                {#if rec.priority}
                  <span
                    class="px-2 py-1 bg-gray-800/50 border border-gray-600/30 text-xs rounded text-gray-300 uppercase tracking-wide"
                  >
              ...
              <p class="text-gray-300 mb-3 leading-relaxed font-mono text-sm">
                {rec.description}
              </p>
              ...
              {#if rec.reason}
                <div class="mb-3">
                  <span
                    class="text-cyan-400 text-sm font-semibold uppercase tracking-wide"
                    >Dlaczego dla Ciebie:</span
                  >
                  <p class="text-gray-400 text-sm italic font-mono">
                    {rec.reason}
                  </p>
                </div>
              {/if}
              ...
              {#if rec.category}
                <div class="flex justify-between items-center">
                  <span
                    class="px-3 py-1 bg-cyan-900/20 border border-cyan-500/30 text-cyan-300 text-xs rounded uppercase tracking-wide font-semibold"
                  >
                    {rec.category}
                  </span>
                  <button
                    class="px-4 py-2 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white text-sm rounded hover:from-cyan-500 hover:to-cyan-600 transition-all duration-300 uppercase tracking-wide font-semibold border border-cyan-500/50"
                  >
                    Dowiedz się więcej
                  </button>
                </div>
    ...
    {#if !loading && recommendations.length === 0 && preferences}
      <div class="text-center py-8 text-gray-500">
        ...
        <p class="font-mono uppercase tracking-wide">
          Brak rekomendacji dla podanych preferencji. Spróbuj ponownie z innymi
          danymi.
        </p>
      </div>
    {/if}
