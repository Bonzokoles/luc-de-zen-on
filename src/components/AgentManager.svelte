<script lang="ts">
  ...
  async function loadAgents() {
    ...
    try {
      ...
      error = 'B³¹d podczas ³adowania agentów: ' + (err as Error).message;
    } finally {
      loading = false;
    }
  }
  ...
  async function createAgent() {
    ...
    try {
      ...
      if (response.ok) {
        success = 'Agent zosta³ utworzony pomyœlnie!';
      ...
      error = 'B³¹d podczas tworzenia agenta: ' + (err as Error).message;
    } finally {
      loading = false;
    }
  }
  ...
  async function updateAgent() {
    ...
    try {
      ...
      if (response.ok) {
        success = 'Agent zosta³ zaktualizowany pomyœlnie!';
        showEditForm = false;
        selectedAgent = null;
        resetForm();
        await loadAgents();
      ...
      error = 'B³¹d podczas aktualizacji agenta: ' + (err as Error).message;
    } finally {
      loading = false;
    }
  }
  ...
  async function deleteAgent(agent: Agent) {
    if (!confirm(`Czy na pewno chcesz usun¹æ agenta "${agent.name}"?`)) return;
    ...
    try {
      ...
      if (response.ok) {
        success = 'Agent zosta³ usuniêty pomyœlnie!';
        await loadAgents();
      } else {
        const errorData = await response.json();
      ...
      error = 'B³¹d podczas usuwania agenta: ' + (err as Error).message;
    } finally {
      loading = false;
    }
  }
          ...
          <div class="specializations">
            ...
            <ul>
              ...
              {#if agent.specializations.length > 3}
                <li class="more">+{agent.specializations.length - 3} wiêcej...</li>
              {/if}
            </ul>
          </div>
        {/if}
      ...
      <div class="modal" on:click|stopPropagation>
        ...
        <form on:submit|preventDefault={createAgent}>
          <div class="form-grid">
            ...
            <div class="form-group">
              <label for="language"> Jêzyk Programowania</label>
          ...
          <div class="form-group">
            ...
            <textarea 
              ...
              required 
              placeholder="Opisz funkcjonalnoœæ i zastosowanie tego agenta..."
              rows="3"
            ></textarea>
          </div>
          ...
          <div class="form-group">
            <label for="enabledFeatures"> W³¹czone Funkcje (oddzielone przecinkami)</label>
            <input 
              type="text" 
              id="enabledFeatures"
