...
const statusColor = (status: string) => {
    switch (status) {
        case 'online': return 'text-emerald-400 bg-emerald-900/30';
...
export default function WorkersStatusDashboard({ className = '' }: WorkersStatusDashboardProps) {
    ...
    const testWorker = async (worker: WorkerStatus) => {
        ...
        try {
            ...
            const updatedWorkers = workers.map(w =>
                w.name === worker.name
                    ? { ...w, status: response.ok ? 'online' as const : 'offline' as const, lastCheck: new Date().toISOString() }
                    : w
            );
            ...
            const updatedWorkers = workers.map(w =>
                w.name === worker.name
                    ? { ...w, status: 'offline' as const, lastCheck: new Date().toISOString() }
                    : w
            );
            setWorkers(updatedWorkers);
        }
    ...
    const avgResponseTime = workers
        .filter(w => w.responseMs !== null)
        .reduce((sum, w) => sum + (w.responseMs || 0), 0) /
        workers.filter(w => w.responseMs !== null).length || 0;

    return (
        <div className={`bg-black/90 backdrop-blur-sm rounded-xl border border-cyan-500/30 p-6 ${className}`}>
            {/* Header */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-900/50 rounded-lg p-4 border border-cyan-800/30">
                    <div className="text-cyan-400 text-sm uppercase tracking-wide">Uptime</div>
                    <div className="text-2xl font-bold text-white">
                        {((onlineWorkers / totalWorkers) * 100).toFixed(1)}%
                    </div>
                </div>
                <div className="bg-gray-900/50 rounded-lg p-4 border border-cyan-800/30">
                    <div className="text-cyan-400 text-sm uppercase tracking-wide">Avg Response</div>
                    <div className="text-2xl font-bold text-white">
                        {avgResponseTime.toFixed(0)}ms
                    </div>
                </div>
                <div className="bg-gray-900/50 rounded-lg p-4 border border-cyan-800/30">
                    <div className="text-cyan-400 text-sm uppercase tracking-wide">Total Requests</div>
                    <div className="text-2xl font-bold text-white">
                        {workers.reduce((sum, w) => sum + w.requests, 0)}
                    </div>
                </div>
                <div className="bg-gray-900/50 rounded-lg p-4 border border-cyan-800/30">
                    <div className="text-cyan-400 text-sm uppercase tracking-wide">Total Errors</div>
                    <div className="text-2xl font-bold text-white">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-md transition-colors duration-200 font-mono text-sm ${autoRefresh
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-cyan-800/50">
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Worker</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Status</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">CPU</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">RAM</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Requests</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Response</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Uptime</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Errors</th>
                            <th className="text-left py-3 px-2 text-cyan-400 font-mono uppercase tracking-wide">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {workers.map((worker, index) => (
                            <tr key={index} className="border-b border-gray-800/30 hover:bg-cyan-900/10 transition-colors">
                                <td className="py-3 px-2">
                                    <div className="font-mono text-white">{worker.name}</div>
                                    <div className="text-xs text-gray-400">{worker.url}</div>
                                </td>
                                <td className="py-3 px-2">
                                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-mono ${statusColor(worker.status)}`}>
                                ...
                                <td className="py-3 px-2 font-mono text-white">
                                    <span className={worker.errors > 0 ? 'text-red-400' : 'text-emerald-400'}>
                                        {worker.errors}
                                    </span>
                                </td>
                                <td className="py-3 px-2">
