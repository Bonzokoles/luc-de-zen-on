...
const TrafficChart: React.FC = () => {
    ...
    useEffect(() => {
        ...
        data.forEach((point, index) => {
            ...
            const y = height - padding - (height - 2 * padding) * point.visitors / maxVisitors;
            if (index === 0) {
                ctx.moveTo(x, y);
            } else {
                ctx.lineTo(x, y);
        ...
        data.forEach((point, index) => {
            const x = padding + (width - 2 * padding) * index / (data.length - 1);
            const y = height - padding - (height - 2 * padding) * point.requests / maxRequests;
            if (index === 0) {
                ctx.moveTo(x, y);
            } else {
                ctx.lineTo(x, y);
        ...
        data.forEach((point, index) => {
            ...
            // Visitors points
            const visitorsY = height - padding - (height - 2 * padding) * point.visitors / maxVisitors;
            ...
            // Requests points
            const requestsY = height - padding - (height - 2 * padding) * point.requests / maxRequests;
    ...
    return (
        <div className="bg-gray-900 border border-cyan-500/30 rounded-lg p-6 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-cyan-300 font-mono">TRAFFIC ANALYSIS</h3>
                ...
                <div className="flex space-x-2">
                    {(['24h', '7d', '30d'] as const).map((range) => (
                        <button
                            ...
                            className={`px-3 py-1 text-sm font-mono border rounded transition-colors ${timeRange === range
            ...
            <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-gray-700">
                ...
                <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-yellow-400">
                        {data.length > 0 ? data.reduce((sum, d) => sum + d.requests, 0).toLocaleString() : 0}
                    </div>
                </div>
                <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-red-400">
                        {data.length > 0 ?
                            ((data.reduce((sum, d) => sum + d.errors, 0) / data.reduce((sum, d) => sum + d.requests, 0)) * 100).toFixed(1) + '%' :
                            '0.0%'
                        }
                    </div>
                </div>
