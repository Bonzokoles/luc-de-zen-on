<script lang="ts">
  ...
  let agents = [
    { id: 'POLACZEK_T', name: 'Tłumacz', type: 'T', role: 'translator', status: 'active', description: '' },
    { id: 'POLACZEK_M1', name: 'Music Assistant 1', type: 'M', role: 'music-player', status: 'idle', description: '' },
    { id: 'POLACZEK_D1', name: 'Dashboard Keeper 1', type: 'D', role: 'dashboard-keeper', status: 'active', description: '' },
    { id: 'POLACZEK_B', name: 'Bibliotekarz', type: 'B', role: 'bibliotekarz', status: 'active', description: '' }
  ];
  ...
  function updateStatus(id: string, status: string) {
    ...
    // TODO: Wywołanie API do aktualizacji statusu na backendzie
  }
  ...
  async function fetchAgentsFromBackend() {
    try {
      ...
      if (resp.ok) {
        ...
        agents = data.map(d => ({ description: '', ...d }));
      }
    } catch (e) {
      console.error("Could not fetch agents from backend", e);
    }
  }
  ...
  <table>
    ...
    <tbody>
      {#each visibleAgents as agent}
        <tr class:active-row={agent.status === "active"}>
          <td>{agent.id}</td>
          <td>{agent.name}</td>
          <td>{agent.type}</td>
          <td>{agent.role}</td>
...
<style>
  ...
  .new-agent-row { display: flex; gap: 9px; margin-bottom: 12px; align-items: center; }
