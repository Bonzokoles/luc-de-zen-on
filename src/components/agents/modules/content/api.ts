...
async function analyzeTextWithGPT4(content: string): Promise<ModerationResult> {
  try {
    ...
    const hasProfanity = polishProfanity.some((word) =>
    ...
    return {
      ...
      severity,
      action,
      explanation: `Analiza wykryła potencjalne problemy z treścią. Ocena: ${(
        maxScore * 100
      ).toFixed(1)}%`,
      suggestedEdits:
        flags.length > 0
...
async function analyzeImageWithRekognition(
  ...
  try {
    ...
    return {
      ...
      severity,
      action,
      explanation: `Analiza obrazu wykryła potencjalne naruszenia. Ocena: ${(
        maxScore * 100
      ).toFixed(1)}%`,
    };
  } catch (error) {
...
${
  variables.keyword || "Kluczowa funkcjonalność"
} oferuje szereg możliwości implementacji w różnych branżach. Polskie firmy coraz częściej inwestują w te rozwiązania.

## Podsumowanie
Wdrożenie ${
          variables.keyword || "omawianych rozwiązań"
        } może przynieść znaczące korzyści biznesowe. ${
          variables.cta || "Skontaktuj się z nami, aby dowiedzieć się więcej."
        }`;
        break;
...
${variables.features || "Zaawansowane funkcjonalności"} w połączeniu z ${
          variables.benefits || "wyjątkowymi korzyściami"
        } sprawiają, że to rozwiązanie jest idealne dla wymagających klientów.
...
${
  variables.keywords || "Kluczowe funkcje"
} zostały zaprojektowane z myślą o polskim rynku.

🛒 **Zamów już dziś i przekonaj się sam!**`;
        break;

      case "email":
        generatedContent = `Temat: ${
          variables.subject || "Specjalna oferta tylko dla Ciebie"
        }

Cześć ${variables.firstName || "Drogi Kliencie"}!

${
  variables.offer || "Mamy dla Ciebie wyjątkową propozycję"
}, która z pewnością Cię zainteresuje.
...
Zespół`;
      ...
      case "seo":
        const title = `${variables.keyword || "Główne słowo kluczowe"} - ${
          variables.brand || "Twoja Marka"
        }`;
        const description = `Poznaj najlepsze ${
          variables.keyword || "rozwiązania"
        } w Polsce. ${
          variables.pageType || "Profesjonalne usługi"
        } z gwarancją jakości. Sprawdź już dziś!`;
...
${description}`;
      ...
      case "newsletter":
        generatedContent = `**Newsletter Tygodniowy - Tydzień ${
...
${
  variables.tipOfWeek ||
  "Regularne analizowanie metryk pomoże Ci lepiej zrozumieć potrzeby klientów."
}
...
Do zobaczenia w przyszłym tygodniu!`;
      ...
      default:
        generatedContent = `Treść wygenerowana na podstawie szablonu: ${
          template.name
        }
  ...
  .join("\n")}`;
    ...
    if (variables.keyword) {
      const keywordCount = (
        generatedContent
          .toLowerCase()
          .match(new RegExp(variables.keyword.toLowerCase(), "g")) || []
      ).length;
      keywordDensity[variables.keyword] = (keywordCount / wordCount) * 100;
    }

    const seoScore = template.seoOptimized
      ? Math.random() * 30 + 70
      : Math.random() * 40 + 40; // 70-100 for SEO optimized, 40-80 for others
    ...
    return {
      ...
      suggestions: [
        ...
        readabilityScore < 80
          ? "Użyj krótszych zdań dla lepszej czytelności"
          : null,
  ...
  try {
    ...
    return {
      ...
      emotions: {
        ...
        sadness:
          sentiment === "negative" ? confidence * 0.7 : Math.random() * 0.2,
...
async function detectSpam(
  content: string
): Promise<{ isSpam: boolean; confidence: number; indicators: string[] }> {
  ...
  const confidence = Math.min(
    0.95,
    foundIndicators.length * 0.15 + Math.random() * 0.2
  );
...
async function getContentPerformanceAnalytics(
  contentId?: string
    ...
    switch (action) {
      ...
      case "status":
        return successResponse({
          ...
          activeModels:
            Object.keys(AGENT_CONFIG.aiModels.moderation).length +
            Object.keys(AGENT_CONFIG.aiModels.generation).length,
        });
      ...
      case "moderate-image":
        ...
        const imageModerationResult = await analyzeImageWithRekognition(
          data.url
        );
        return successResponse(
          imageModerationResult,
          "Moderacja obrazu zakończona"
        );
      ...
      case "get-moderation-queue":
        ...
        return successResponse(
          queue,
          `Znaleziono ${queue.length} elementów w kolejce moderacji`
        );
      ...
      case "generate-content":
        ...
        const template = AGENT_CONFIG.contentTemplates.find(
          (t) => t.id === data.template
        );
        if (!template) {
      ...
      case "create-blog-post":
        ...
        const blogTemplate = AGENT_CONFIG.contentTemplates.find(
          (t) => t.id === "blog-post-seo"
        );
        if (!blogTemplate) {
          return errorResponse("Szablon artykułu blogowego nie jest dostępny");
        }

        const blogResult = await generateContentWithAI(
          blogTemplate,
          data,
          data.options || {}
        );
        return successResponse(
          blogResult,
          "Artykuł blogowy wygenerowany pomyślnie"
        );
      ...
      case "social-media-content":
        ...
        const socialTemplate = AGENT_CONFIG.contentTemplates.find(
          (t) => t.id === "social-media-post"
        );
        if (!socialTemplate) {
          return errorResponse("Szablon posta social media nie jest dostępny");
        }

        const socialResult = await generateContentWithAI(
          socialTemplate,
          data,
          data.options || {}
        );
        return successResponse(
          socialResult,
          "Post social media wygenerowany pomyślnie"
        );
      ...
      case "product-descriptions":
        ...
        const productTemplate = AGENT_CONFIG.contentTemplates.find(
          (t) => t.id === "product-description"
        );
        if (!productTemplate) {
          return errorResponse("Szablon opisu produktu nie jest dostępny");
        }

        const productResult = await generateContentWithAI(
          productTemplate,
          data,
          data.options || {}
        );
        return successResponse(
          productResult,
          "Opis produktu wygenerowany pomyślnie"
        );
      ...
      case "email-campaigns":
        ...
        const emailTemplate = AGENT_CONFIG.contentTemplates.find(
          (t) => t.id === "email-campaign"
        );
        if (!emailTemplate) {
          return errorResponse("Szablon kampanii email nie jest dostępny");
        }

        const emailResult = await generateContentWithAI(
          emailTemplate,
          data,
          data.options || {}
        );
        return successResponse(
          emailResult,
          "Kampania email wygenerowana pomyślnie"
        );

      case "seo-meta-tags":
        if (!data?.topic || !data?.keyword) {
          return errorResponse(
            "Brak tematu lub słowa kluczowego dla meta tagów"
          );
        }

        const seoTemplate = AGENT_CONFIG.contentTemplates.find(
          (t) => t.id === "seo-meta-tags"
        );
        if (!seoTemplate) {
          return errorResponse("Szablon meta tagów nie jest dostępny");
        }

        const seoResult = await generateContentWithAI(
          seoTemplate,
          data,
          data.options || {}
        );
      ...
      case "get-content-performance":
        const performance = await getContentPerformanceAnalytics(
          data?.contentId
        );
        return successResponse(
          performance,
          "Dane wydajności treści pobrane pomyślnie"
        );
        ...
        return successResponse(
          {
            ...
            keywordDensity: sortedKeywords.map(([word, count]) => ({
              ...
              count,
              density: (
                ((count as number) / data.content.split(/\s+/).length) *
                100
              ).toFixed(2),
            })),
            seoRecommendations: [
      ...
      case "ab-test-results":
        ...
        return successResponse(
          abTestResults,
          "Wyniki testu A/B pobrane pomyślnie"
        );
