...
async function processVoiceCommand(data: any) {
    ...
    if (matchedCommand) {
        ...
        return createResponse({
            ...
            transcript,
            confidence,
            message: 'Nie rozpoznano komendy. Spróbuj: "czas", "data", "idź do dashboard"',
...
function handleNavigation(text: string) {
    ...
    if (text.includes('dashboard')) {
        return { action: 'navigate', url: '/dashboard', message: 'Przechodzę do dashboard' };
    }
    if (text.includes('voice') || text.includes('głos')) {
        return { action: 'navigate', url: '/voice-ai', message: 'Przechodzę do Voice AI' };
    }
    if (text.includes('agentów') || text.includes('agents')) {
        return { action: 'navigate', url: '/agents', message: 'Przechodzę do agentów' };
    }
    if (text.includes('home') || text.includes('główna')) {
        return { action: 'navigate', url: '/', message: 'Przechodzę do strony głównej' };
    }

    return {
        action: 'navigation_help',
        message: 'Podaj dokładną lokalizację. Dostępne: dashboard, voice, agents, home'
    };
}
...
function getSuggestions(text: string) {
    ...
    const suggestions = [
        'czas', 'data', 'idź do dashboard', 'uruchom agenta', 'lista agentów'
    ];

    return suggestions.filter(suggestion =>
        suggestion.includes(text) || text.includes(suggestion)
    ).slice(0, 3);
}
