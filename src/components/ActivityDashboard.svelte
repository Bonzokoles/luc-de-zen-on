<script lang="ts">
  ...
  async function fetchLogs() {
    try {
      loading = true;
      const url = selectedType === 'all' ? '/api/activity-monitor' : `/api/activity-monitor?type=${selectedType}`;
  ...
  async function logSampleActivity() {
    try {
      const sampleActivities = [
        {
          ...
          type: 'info',
          source: 'auth',
          details: { ip: '192.168.1.100', browser: 'Firefox' }
        },
        {
          action: 'API rate limit exceeded',
          type: 'warning',
          source: 'api',
          details: { endpoint: '/api/generate-content', limit: 100 }
        },
        {
        ...
        },
        {
          ...
          type: 'info',
          source: 'ai',
          details: { model: 'gpt-4o-mini', tokens: 1200 }
        }
      ];
...
<div class="bg-black/80 backdrop-blur-sm border border-cyan-500/30 rounded-xl p-6 text-cyan-300 max-h-[800px] overflow-hidden">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-4 text-center">
  ...
  {#if stats.anomalies}
    <div class="mb-6 p-4 bg-red-900/20 border border-red-500/50 rounded-lg">
      ...
      <p class="text-red-300 mt-2">Wykryto nietypową liczbę błędów w systemie. Sprawdź logi poniżej.</p>
    </div>
  {/if}
        ...
        <div class="bg-gray-900/30 border border-gray-700/50 rounded-lg p-4 hover:border-cyan-500/50 transition-colors">
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <div class="flex items-center space-x-2">
                  <span class="font-semibold {typeColors[log.type]}">{log.action}</span>
                  <span class="text-xs bg-gray-700 px-2 py-1 rounded text-gray-300">{log.source}</span>
                </div>
          ...
          {#if Object.keys(log.details).length > 0}
            <div class="mt-3 p-2 bg-gray-800/50 rounded text-xs">
              ...
              <pre class="text-cyan-200 whitespace-pre-wrap">{JSON.stringify(log.details, null, 2)}</pre>
            </div>
          {/if}
        </div>
      {/each}
