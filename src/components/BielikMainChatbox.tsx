...
export default function BielikMainChatbox({
  ...
  const sessionId = useRef(
    `bielik_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  );
    ...
    setMessages([
      {
        ...
        role: "system",
        content:
          "🇵🇱 BIELIK Orchestrator Online - Główny AI systemu MyBonzo gotowy do pracy!",
        timestamp: new Date(),
      },
    ]);
  }, []);
    ...
    try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            content: `✅ BIELIK połączony pomyślnie!\n🧠 Aktywne modele: ${
              Object.keys(data.connectedModels || {}).length
            }\n⚡ Wersja: ${
              data.version
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "system",
          content:
            "❌ Błąd połączenia z BIELIK. Sprawdź konfigurację orkiestratora.",
          timestamp: new Date(),
        },
      ]);
    }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: `error_${Date.now()}`,
        role: "system",
        content: `❌ Błąd komunikacji z ${selectedModel.name}: ${
  ...
  const getVariantStyles = () => {
    switch (variant) {
      case "floating":
        return {
          container:
            "fixed bottom-4 right-4 w-96 h-128 bg-black/90 border-2 border-cyan-400 shadow-2xl z-50",
          chat: "h-80",
        };
      case "fullscreen":
        return {
      ...
      default:
        return {
          container:
            "w-full max-w-4xl mx-auto bg-black/80 border-2 border-cyan-400 shadow-xl",
          chat: "h-96",
        };
    }
  };
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-yellow-400 font-bold text-lg">
              BIELIK - Główny AI Chatbox
            </h3>
            <p className="text-yellow-200 text-sm">
              Orkiestrator wszystkich modeli AI
            </p>
          </div>
        </div>
      ...
      <div className={`${styles.chat} overflow-y-auto p-4 bg-black/40`}>
                  ...
                  <div className="mt-3 pt-2 border-t border-current/20">
                    <div className="text-xs opacity-80">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {message.orchestratorInfo.activatedModels.map(
                          (model, idx) => (
                            <span
                              key={idx}
        ...
        {isLoading && (
          <div className="flex justify-start mb-4">
            <div
              className="bg-yellow-900/30 border-2 border-yellow-400 text-yellow-100 px-4 py-3 max-w-3xl"
              style={{ borderRadius: "0px" }}
            >
              <div className="flex items-center gap-2">
      ...
      <div className="border-t-2 border-cyan-400 p-3 bg-black/70 relative">
        ...
        {showModelSelector && (
          ...
          >
            <div className="p-3">
              <h4 className="text-yellow-400 font-bold mb-3 text-sm">
                🇵🇱 POLSKIE MODELE AI (Cloudflare)
              </h4>
                ...
                <button
                  ...
                  className={`w-full text-left p-3 border-b border-cyan-400/30 hover:bg-cyan-900/30 transition-colors ${
                    selectedModel.id === model.id
                      ? "bg-yellow-900/30 border-l-4 border-l-yellow-400"
                      : ""
                  }`}
                  style={{ borderRadius: "0px" }}
                >
      ...
      <div className="border-t-2 border-cyan-400 p-4 bg-black/60">
        ...
        <div className="mt-3 flex justify-between items-center text-xs">
          <div className="flex items-center gap-4">
            <span className="text-cyan-300">
              ...
              <span
                className={
                  bielikStatus === "active" ? "text-green-400" : "text-red-400"
                }
              >
                {bielikStatus}
              </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-yellow-400">🧠 AI Orchestrator Active</span>
          </div>
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <button
            onClick={() =>
              setInputValue("Zoptymalizuj wydajność wszystkich workers")
            }
