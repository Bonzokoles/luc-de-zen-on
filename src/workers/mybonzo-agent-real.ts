...
export class MyBonzoAgent extends Agent<MyBonzoAgentEnv, MyBonzoAgentState> {
  ...
  async chat(message: string, model?: string) {
    ...
    try {
      ...
      return {
        response:
          "⚠️ Przepraszam, wystąpił błąd podczas przetwarzania wiadomości.",
        error: true,
        timestamp: new Date().toISOString(),
      };
    }
  ...
  async generateImage(prompt: string, style: string = "cyberpunk") {
    try {
      ...
      if (this.env.IMAGES && imageResponse) {
        ...
        await this.env.IMAGES.put(imageKey, imageResponse as any);
  ...
  async translate(text: string, targetLanguage: string = "en") {
    try {
      const response = await this.env.AI.run("@cf/meta/m2m100-1.2b" as any, {
        text,
        source_lang: "polish",
      ...
      return {
        originalText: text,
        translatedText: (response as any).translated_text,
        targetLanguage,
        timestamp: new Date().toISOString(),
        agentId: this.id,
      };
  ...
  async updatePreferences(
    preferences: Partial<typeof this.initialState.userPreferences>
  ) {
    await this.setState({
      userPreferences: {
      ...
      switch (taskType) {
        case "research":
          result = await this.chat(
            `Zbadaj temat: ${taskData.topic}. Podaj szczegółowe informacje.`
          );
          break;

        case "summarize":
          result = await this.chat(
            `Podsumuj następujący tekst w 3-5 punktach: ${taskData.text}`
          );
          break;

        case "creative":
          result = await this.chat(
            `Stwórz kreatywną treść na temat: ${taskData.prompt}. Styl: ${
              taskData.style || "nowoczesny"
            }`
          );
          break;
