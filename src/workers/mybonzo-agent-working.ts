...
export class MyBonzoAgent extends Agent<MyBonzoAgentEnv, MyBonzoAgentState> {
  ...
  async generateImage(prompt: string, style: string = "cyberpunk") {
    try {
      ...
      await this.env.IMAGES.put(imageKey, JSON.stringify({
        prompt: enhancedPrompt,
        style,
  ...
  async executeTask(taskType: string, taskData: any) {
    try {
      ...
      switch (taskType) {
        case "research":
          result = await this.chat(`🔍 Zbadaj temat: ${taskData.topic}. Podaj szczegółowe informacje w stylu MyBonzo.`);
          break;
        
        case "summarize":
          result = await this.chat(`📋 Podsumuj w 3-5 punktach: ${taskData.text}`);
          break;
        
        case "creative":
          result = await this.chat(`🎨 Stwórz kreatywną treść: ${taskData.prompt}. Styl: ${taskData.style || 'cyberpunk'}`);
          break;
        
        case "code":
          result = await this.chat(`💻 Pomóż z kodem: ${taskData.request}. Wyjaśnij i podaj przykład.`);
          break;
  ...
  // Ustawienia
  async updatePreferences(preferences: Partial<MyBonzoAgentState["userPreferences"]>) {
    this.setState({
      ...this.state,
      userPreferences: {
  ...
  async saveToKV() {
    ...
    await this.env.AGENTS.put(`agent-${agentData.id}`, JSON.stringify(agentData));
