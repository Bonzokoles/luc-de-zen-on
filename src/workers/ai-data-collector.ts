...
export default {
  async fetch(request: Request, env: any): Promise<Response> {
    ...
    try {
      ...
      if (
        url.pathname === "/api/data-collector/start" &&
        request.method === "POST"
      ) {
      ...
      if (
        url.pathname === "/api/data-collector/status" &&
        request.method === "GET"
      ) {
      ...
      // Get storage info
      if (
        url.pathname === "/api/data-collector/storage" &&
        request.method === "GET"
      ) {
      ...
      if (
        url.pathname === "/api/data-collector/schedule" &&
        request.method === "POST"
      ) {
      ...
      if (
        url.pathname === "/api/data-collector/topics" &&
        request.method === "GET"
      ) {
...
async function handleDataCollection(
  ...
  // Store job info
  try {
    await env.DATA_COLLECTION_JOBS?.put(
      collectionId,
      JSON.stringify(collectionJob),
      {
...
async function processDataCollectionInBackground(
  job: any,
  env: any
): Promise<void> {
  ...
  for (const topic of topics) {
    try {
      ...
      const searchResults = await searchTopicData(
        topic,
        sources,
        maxFilesPerTopic,
        env
      ...
      for (const result of searchResults) {
        try {
          ...
          job.progress = Math.min(
            100,
            (collectedFiles / (topics.length * maxFilesPerTopic)) * 100
          );
...
async function searchTopicData(
  topic: string,
  sources: string[],
  maxResults: number,
  env: any
  ...
  for (const source of sources) {
    try {
      ...
      if (source === "tavily") {
        searchResults = await searchWithTavily(
          topic,
          maxResults / sources.length,
          env
        );
      } else if (source === "web") {
        searchResults = await searchWithWebAPI(
          topic,
          maxResults / sources.length,
          env
        );
      } else if (source === "kaggle") {
        searchResults = await searchWithKaggle(
          topic,
          maxResults / sources.length,
          env
        );
...
async function searchWithKaggle(
  topic: string,
  maxResults: number,
  env: any
): Promise<any[]> {
...
async function saveToGoogleDrive(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to upload to Google Drive: ${response.statusText}`
      );
    }
...
async function createFolderIfNotExists(
  ...
  const searchResponse = await fetch(
    `https://www.googleapis.com/drive/v3/files?q=name='${folderPath}' and mimeType='application/vnd.google-apps.folder'`,
    {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
...
async function handleScheduleCollection(
  ...
  try {
    await env.COLLECTION_SCHEDULES?.put(
      scheduleConfig.id,
...
async function handleGetTopics(env: any, corsHeaders: any): Promise<Response> {
  ...
  return new Response(
    JSON.stringify({
      ...
      categories: {
        "Technology & AI": topicsWithStats.filter(
          (t) => t.category === "Technology & AI"
        ).length,
        "Business & Marketing": topicsWithStats.filter(
          (t) => t.category === "Business & Marketing"
        ).length,
        Development: topicsWithStats.filter((t) => t.category === "Development")
          .length,
        Industry: topicsWithStats.filter((t) => t.category === "Industry")
          .length,
      },
    }),
    {
