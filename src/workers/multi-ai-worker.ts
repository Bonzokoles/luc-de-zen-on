...
interface MultiAIRequest {
  provider: 'openai' | 'anthropic' | 'deepseek' | 'perplexity' | 'google-ai-studio' | 'huggingface' | 'elevenlabs';
  model?: string;
  messages?: Array<{
    role: string;
    content: string;
    ...
    try {
      ...
      switch (requestData.provider) {
        ...
        case 'huggingface':
          ...
          body = {
            inputs: requestData.inputs || requestData.messages?.[0]?.content || '',
            parameters: {
      ...
      if (!response.ok) {
        console.error(`${requestData.provider} API error:`, response.status, await response.text());
        return new Response(
          JSON.stringify({ 
