...
export class RAGAgent {
  ...
  async search(request: RAGSearchRequest): Promise<RAGSearchResult> {
    ...
    try {
      ...
      // Search Vectorize
      const searchResults = await this.env.VOICE_EMBEDDINGS.query(
        queryEmbedding,
        {
      ...
      if (request.threshold) {
        filteredResults = filteredResults.filter(
          (match: any) => match.score >= request.threshold!
        );
      }

      // Format results
      const documents = filteredResults.map((match: any) => ({
        id: match.id,
        content:
      ...
      console.log(
        `✅ RAG search completed: ${documents.length} results in ${processingTime}ms`
      );
  ...
  async generateContextualResponse(
    ...
    try {
      ...
      const ragContext = searchResults.documents
        .map(
          (doc) =>
            `Source: ${doc.metadata.title || doc.id}\nContent: ${doc.content}`
        )
        .join("\n\n");
  ...
  async updateDocument(
    ...
    try {
      ...
      if (updates.content && updates.content !== existing.content) {
        updatedDocument.embedding = await this.generateEmbedding(
          updates.content
        );
      }
