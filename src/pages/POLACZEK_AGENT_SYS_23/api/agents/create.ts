...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    console.log(
      `[AGENT CREATE] New agent created: ${name} (${type}) and added to list`
    );
...
async function generateAgentConfig(agentData: any) {
  const {
    name,
    type,
    description,
...
async function generateAgentCode(agentData: any, config: any): Promise<string> {
...
function generatePythonAgent(config: any): string {
  const separator = Array(51).join("=");
  const capabilitiesList = config.capabilities || [];
  const capabilitiesText = capabilitiesList
    .map((cap: string) => `- ${cap.replace("_", " ")}`)
    .join("\n");
...
function generateNodeJSAgent(config: any, isTypeScript: boolean): string {
...
async function createAgentFiles(
  name: string,
  type: string,
  config: any,
  code: string
  ...
  const scriptFileName = `${name.toLowerCase()}.${
    config.language === "python" ? "py" : "js"
  }`;

  // Simulate file creation
  console.log(
    `[FILE CREATE] ${configFileName} - ${JSON.stringify(config).length} bytes`
  );
...
function generatePortFromName(name: string): number {
  let hash = 0;
...
function generateDefaultSystemPrompt(type: string): string {
  const prompts = {
    chatbot:
      "Jesteś przyjaznym i pomocnym asystentem AI. Odpowiadaj w sposób naturalny i profesjonalny.",
    translator:
      "Jesteś ekspertem w tłumaczeniu języków. Tłumacz tekst dokładnie zachowując kontekst.",
    searcher:
      "Jesteś specjalistą od wyszukiwania informacji. Znajdź najbardziej aktualne informacje.",
    monitor:
      "Jesteś agentem monitorującym system. Analizuj metryki i alarmuj o problemach.",
    artist:
      "Jesteś kreatywnym asystentem AI specjalizującym się w generowaniu obrazów.",
    analyst:
      "Jesteś analitykiem danych. Analizuj informacje i twórz szczegółowe raporty.",
    writer: "Jesteś profesjonalnym pisarzem. Twórz wysokiej jakości treści.",
    coder:
      "Jesteś doświadczonym programistą. Pomagaj w pisaniu i debugowaniu kodu.",
    scheduler:
      "Jesteś zarządcą zadań. Organizuj i planuj działania efektywnie.",
    custom:
      "Jesteś wszechstronnym agentem AI. Dostosowuj się do różnych zadań.",
  };
