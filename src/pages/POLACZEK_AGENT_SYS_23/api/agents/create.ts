...
function generatePythonAgent(config: any): string {
    ...
    const capabilitiesText = capabilitiesList.map((cap: string) => `- ${cap.replace('_', ' ')}`).join('\n');
...
async function createAgentFiles(name: string, type: string, config: any, code: string) {
