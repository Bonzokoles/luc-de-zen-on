...
export const POST: APIRoute = async ({ request }) => {
  try {
    const { message, history, language, context, features } =
    ...
    const { language: requestLanguage } = (await request
      .json()
      .catch(() => ({ language: "pl-PL" }))) as any;
...
async function generateEnhancedAIResponse(message: string, options: any) {
  ...
  if (
    ...
    lowerMessage.includes("głos") ||
    lowerMessage.includes("mowa")
  ) {
  ...
  if (
    ...
    lowerMessage.includes("sztuczna inteligencja")
  ) {
  ...
  if (
    ...
    lowerMessage.includes("analityka")
  ) {
...
function generatePlatformResponse(message: string, language: string) {
  ...
  const langResponses =
    responses[language as keyof typeof responses] || responses["en-US"];
...
function getFallbackResponse(language: string) {
  const fallbacks = {
    "pl-PL":
      "Przepraszam, wystąpił problem techniczny. Spróbuj ponownie lub skorzystaj z innych funkcji MyBonzo.",
    "en-US":
      "Sorry, there was a technical issue. Please try again or use other MyBonzo functions.",
  };
