...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    const { dataset, analysisType, customPrompt } =
    ...
    const auth = Buffer.from(`${kaggleUsername}:${kaggleKey}`).toString(
      "base64"
    );
    ...
    const kaggleData = (await kaggleResponse.json()) as any;
    ...
    switch (analysisType) {
      case "quick":
        analysisPrompt =
          "Provide a quick analysis focusing on data structure, key features, and potential applications.";
        break;
      case "detailed":
        analysisPrompt =
          "Provide a comprehensive analysis including data quality assessment, statistical insights, potential biases, and detailed recommendations for analysis approaches.";
        break;
      case "ml":
        analysisPrompt =
          "Focus on machine learning potential: suitable algorithms, feature engineering opportunities, prediction targets, and expected challenges.";
        break;
      case "custom":
        analysisPrompt =
          customPrompt || "Provide a general analysis of this dataset.";
        break;
      default:
...
Files: ${
      datasetDetails.files
        ? datasetDetails.files.map((f: any) => f.name).join(", ")
        : "Unknown"
    }
...
5. Key Considerations
    ...
    const analysis =
      deepseekData.choices[0]?.message?.content || "No analysis generated";
