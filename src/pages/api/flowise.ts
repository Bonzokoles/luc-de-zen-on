export async function post({ request }: { request: Request }) {
  try {
    const { prompt, workflowId } = (await request.json()) as any;
    ...
    const FLOWISE_API_URL =
      import.meta.env.FLOWISE_API_URL || "https://api.flowise.com/api/v1";
    ...
    const flowiseEndpoint = `${FLOWISE_API_URL}/chatflows/${
      workflowId || "default"
    }/prediction`;
  ...
  return new Response(
    JSON.stringify({
      ...
      configuration: {
        ...
        FLOWISE_API_TOKEN: import.meta.env.FLOWISE_API_TOKEN
          ? "configured"
          : "not configured",
      },
    }),
