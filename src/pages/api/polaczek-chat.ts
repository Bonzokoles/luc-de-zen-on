...
export const GET = async () => {
  return createSuccessResponse({
    ...
    description:
      'POST { prompt, model?: "qwen"|"gemma"|"fast"|"advanced", temperature?, language?: "pl" }',
    features: [
...
function getContextualResponse(userQuery: string): string {
  ...
  if (
    queryLower.includes("obraz") ||
    queryLower.includes("generuj") ||
    queryLower.includes("zdjęcie") ||
    ...
    context += `• Funkcje: ${MyBonzoKnowledge.services[
      "AI Image Generator"
    ].features.join(", ")}`;
  } else if (
    queryLower.includes("bigquery") ||
    queryLower.includes("analiza") ||
    ...
    queryLower.includes("agents") ||
    queryLower.includes("agent") ||
    queryLower.includes("mybonzo agent")
  ) {
    ...
    queryLower.includes("strona") ||
    queryLower.includes("mybonzo") ||
    queryLower.includes("portfolio") ||
    queryLower.includes("o stronie")
  ) {
    context = `🚀 **O MyBonzo**\n\n`;
    ...
    queryLower.includes("technologie") ||
...
function buildSystemPrompt(
  language: "pl" | "auto" | "en" | undefined,
  context: string
) {
  const lang = language === "en" ? "en" : "pl";

  const sysPl = `Jesteś POLACZEK — polskim AI asystentem dla platformy MyBonzo AI z poznańskim charakterem.
...
🏠 POZNAŃSKA GWARA (używaj sporadycznie dla charakteru):
• "bimba" = tramwaj
• "wuchta" = dużo, mnóstwo  
• "wiara" = prawda, rzeczywiście
• "fura" = samochód
• "szama" = jedzenie
• "knajpa" = restauracja, bar
• "deka" = moment, chwila
• "stara/stary" = kobieta/mężczyzna
...
• Sporadycznie wpleć poznańskie zwroty dla charakteru (max 1-2 na odpowiedź)
• Jeśli nie wiesz - powiedz "Nie mam tej informacji"

🚀 STYL: Ekspert AI, przyjazny, konkretny, praktyczny z poznańskim charakterem.`;
  ...
  try {
    ...
    if (!userInput || typeof userInput !== "string") {
      return createErrorResponse(
        'Pole "prompt" lub "message" jest wymagane',
        400
      );
    }
    ...
    if (!env?.AI) {
      return createErrorResponse(
        "Cloudflare AI nie jest dostępny w środowisku",
        500
      );
    }
    ...
    const aiResp: any = await env.AI.run(
      modelId as any,
      {
        messages,
        temperature,
    ...
    const answer: string =
      aiResp?.response || aiResp?.result || "Brak odpowiedzi od modelu.";
