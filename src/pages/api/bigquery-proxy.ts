...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    // Get environment configuration - use our project ID directly for now
    const projectId = 'zenon-project-467918';
    
    // Optional: Check for GCP_FUNCTION_TOKEN for security
    const env = import.meta.env.DEV ? process.env : (locals as any)?.runtime?.env || {};
    const functionToken = env.GCP_FUNCTION_TOKEN;
