...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: "Required fields are missing from the request body.",
        }),
        { status: 400 }
      );
    }
    ...
    const systemPrompt =
      "You are an experienced and encouraging teacher providing personalized feedback to a student after they've answered a quiz question. Your response must be in Polish.";
    ...
    const feedback =
      aiResponse.response ||
      (isCorrect
        ? "Dobra robota! To poprawna odpowiedź."
        : "Niestety, to nie jest poprawna odpowiedź. Spróbuj jeszcze raz!");
