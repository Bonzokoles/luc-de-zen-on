...
async function getOpenRouterInstructions() {
  return {
    ...
    examples: {
      "Darmowy model":
        '{"message": "Hello!", "model": "meta-llama/llama-3.1-8b-instruct"}',
      "Premium model":
        '{"message": "Analyze this data", "model": "anthropic/claude-3.5-sonnet"}',
      "Z systemem":
        '{"message": "Write code", "system_prompt": "You are a coding expert", "model": "microsoft/wizardlm-2-8x22b"}',
      Streaming:
        '{"message": "Tell a story", "model": "google/gemma-2-9b-it", "stream": true}',
    },
    best_practices: {
...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    if (models) {
      return new Response(
        JSON.stringify({
          ...
          free_count: Object.values(OPENROUTER_MODELS).filter((m) => m.free)
            .length,
          paid_count: Object.values(OPENROUTER_MODELS).filter((m) => !m.free)
            .length,
          timestamp: new Date().toISOString(),
        }),
        {
    ...
    if (conversationId) {
      const conversation = openrouterHistory.find(
    ...
    return new Response(
      JSON.stringify({
        ...
        help: {
          description:
            "Unified API do modeli od OpenAI, Anthropic, Meta, Google i innych",
          total_models: Object.keys(OPENROUTER_MODELS).length,
          free_models: Object.values(OPENROUTER_MODELS).filter((m) => m.free)
            .length,
          features: [
            "Multi-model access",
            "Cost tracking",
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (!OPENROUTER_MODELS[model as keyof typeof OPENROUTER_MODELS]) {
      return new Response(
        JSON.stringify({
          ...
          free_models: Object.keys(OPENROUTER_MODELS).filter(
            (k) => OPENROUTER_MODELS[k as keyof typeof OPENROUTER_MODELS].free
          ),
        }),
        {
          status: 400,
    ...
    let conversation = conversation_id
      ? openrouterHistory.find((c) => c.id === conversation_id)
      : null;
    ...
    const openrouterApiKey =
      env?.OPENROUTER_API_KEY || "OPENROUTER_API_KEY_SECRET";
      ...
      try {
        ...
        estimatedCost =
          (inputTokens * modelInfo.pricing.prompt +
            outputTokens * modelInfo.pricing.completion) /
          1000;
      } catch (error) {
        aiResponse = `Błąd OpenRouter API: ${
          error instanceof Error ? error.message : "Nieznany błąd"
        }`;
      }
    } else {
      aiResponse = await generateMockOpenRouterResponse(
        model,
        message,
        conversation.messages
      );
...
async function generateMockOpenRouterResponse(
  model: string,
  message: string,
  history: any[]
): Promise<string> {
...
Koszt: $${modelInfo.pricing.prompt}/1K prompt + $${
