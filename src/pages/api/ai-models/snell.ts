...
async function getSnellInstructions() {
  return {
    ...
    examples: {
      ...
      "Pomoc w kodzie":
        '{"message": "Jak napisać pętlę for w JavaScript?", "model": "snell-code"}',
      "Kreatywne pisanie":
        '{"message": "Napisz krótką historię o robocie", "model": "snell-creative", "temperature": 0.9}',
      "Z systemem":
        '{"message": "Przeanalizuj ten kod", "system_prompt": "Jesteś ekspertem programowania", "model": "snell-code"}',
    },
    best_practices: {
      models:
        "snell-chat do rozmów, snell-code do programowania, snell-creative do pisania",
...
function generateChatId(): string {
  return (
    "snell_" + Date.now().toString(36) + Math.random().toString(36).substr(2)
  );
}
...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const env =
      (locals as any)?.runtime?.env ?? (globalThis as any).cloudflareEnv ?? {};
    ...
    if (conversationId) {
      const conversation = snellChatHistory.find(
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let conversation = conversation_id
      ? snellChatHistory.find((c) => c.id === conversation_id)
      : null;
    ...
    let aiResponse = await generateSnellResponse(
      model,
      message,
...
async function generateSnellResponse(
  model: string,
  message: string,
  history: any[],
  options: any
