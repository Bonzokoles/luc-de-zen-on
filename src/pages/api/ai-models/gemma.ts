...
async function getGemmaInstructions() {
  return {
    ...
    examples: {
      ...
      'Pomoc w kodzie': '{"message": "Napisz funkcję sortującą w Python", "system_prompt": "Jesteś ekspertem programowania"}',
      'Analiza tekstu': '{"message": "Przeanalizuj sentiment tego tekstu: [tekst]", "temperature": 0.3}',
      'Kontynuacja rozmowy': '{"message": "A co z tym pomysłem?", "conversation_id": "conv_123"}'
    },
    best_practices: {
...
async function getGemmaAIHelp(env: any, question: string) {
  try {
    ...
    return `Błąd Gemma: ${error instanceof Error ? error.message : 'Nieznany błąd'}`;
  }
}
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let conversation = conversation_id ? 
      conversationHistory.find(c => c.id === conversation_id) : null;
    ...
    if (env?.AI) {
      ...
      aiResponse = 'Model Gemma obecnie niedostępny - brak konfiguracji Cloudflare AI';
    }
