    ...
    switch (action) {
      ...
      default:
        return createErrorResponse(
          "Invalid action. Supported: status, validate, debug, providers",
          400
        );
    }
  } catch (error) {
...
async function handleStatus(secretsAPI: MySecretsAPI) {
  const validation: ProviderValidation[] =
    await secretsAPI.validateProviderKeys();
  const debugInfo: SourceDebug[] = await secretsAPI.debugSecretSources();

  const summary = {
    totalProviders: validation.length,
    providersWithKeys: validation.filter((p: ProviderValidation) => p.hasKey)
      .length,
...
async function handleValidate(secretsAPI: MySecretsAPI) {
  ...
  return createSuccessResponse({
    ...
    missingProviders: validation
      .filter((p: ProviderValidation) => !p.hasKey)
      .map((p: ProviderValidation) => p.provider),
  });
}
