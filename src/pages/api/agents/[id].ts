...
export const GET: APIRoute = async ({ params, locals }) => {
  ...
  if (!id) {
    return new Response(JSON.stringify({ error: "Brak ID agenta" }), {
      status: 400,
    });
  }
  ...
  try {
    ...
    const agentData = await (kv as any)?.get(agentKey);

    if (!agentData) {
      return new Response(JSON.stringify({ error: "Nie znaleziono agenta" }), {
        status: 404,
      });
    }
