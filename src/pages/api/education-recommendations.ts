...
const courseCatalog = [
  {
    ...
    level: "advanced",
    description:
      "Głębokie zanurzenie w zaawansowane wzorce projektowe i techniki w JavaScript.",
  },
  {
    id: "ml_fun",
    ...
    level: "beginner",
    description:
      "Podstawy uczenia maszynowego, od regresji po proste sieci neuronowe.",
  },
  {
    id: "k8s_mastery",
    ...
    category: "DevOps",
    level: "advanced",
    description: "Kompletny kurs orkiestracji kontenerów dla profesjonalistów.",
  },
  {
    id: "react_basics",
    ...
    category: "Frontend",
    level: "beginner",
    description: "Zbuduj swoją pierwszą interaktywną aplikację w React.",
  },
  {
    id: "python_ds",
    ...
    level: "intermediate",
    description:
      "Praktyczne wykorzystanie bibliotek Pandas, NumPy i Scikit-learn.",
  },
  {
    id: "node_backend",
    ...
    level: "intermediate",
    description:
      "Tworzenie wydajnych i skalowalnych aplikacji serwerowych w Node.js.",
  },
  {
    id: "css_grid",
    ...
    category: "Frontend",
    level: "beginner",
    description: "Opanuj nowoczesne techniki layoutu w CSS.",
  },
  {
    id: "prompt_eng",
    ...
    category: "AI",
    level: "intermediate",
    description: "Naucz się tworzyć efektywne prompty dla modeli językowych.",
  },
];
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const { skillLevel, interests } = (await request.json()) as any;
    ...
    if (
      ...
      return createErrorResponse(
        "Poziom umiejętności i zainteresowania są wymagane.",
        400
      );
    }
    ...
    const systemPrompt = `
      ...
      ${courseCatalog
        .map(
          (c) =>
            `- ${c.id}, ${c.name}, ${c.category}, ${c.level}, ${c.description}`
        )
        .join("\n")}
    ...
    if (!result.recommendations || !Array.isArray(result.recommendations)) {
      return createErrorResponse(
        "AI zwróciło nieprawidłowy format danych.",
        500
      );
    }

    // Połącz wyniki AI z pełnymi danymi kursów
    const populatedRecommendations = result.recommendations
      .map((rec: any) => {
    ...
    const errorMessage = (error as any).message || "Nieznany błąd serwera.";
