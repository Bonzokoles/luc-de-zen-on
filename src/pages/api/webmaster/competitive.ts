...
export const POST: APIRoute = async ({ request }) => {
  try {
    const { domain, analysis = [] } = (await request.json()) as any;

    if (!domain) {
      return createErrorResponse(
        "Domena jest wymagana do analizy konkurencyjnej",
        400
      );
    }
    ...
    return createErrorResponse(
      `Błąd analizy konkurencyjnej: ${error.message}`,
      500
    );
  }
};
...
function generateKeywords(domain: string) {
  ...
  return allKeywords
    ...
    .map((keyword) => ({
      ...
      trend:
        Math.random() > 0.5 ? "up" : Math.random() > 0.3 ? "stable" : "down",
    }))
...
function generateTrafficData(domain: string) {
  ...
  return {
    ...
    avgSessionDuration: `${Math.floor(Math.random() * 4 + 1)}:${Math.floor(
      Math.random() * 60
    )
      .toString()
      .padStart(2, "0")}`,
  ...
  return {
    ...
    topReferrers: [
      { domain: "github.com", links: Math.floor(Math.random() * 200 + 50) },
      {
        ...
        links: Math.floor(Math.random() * 150 + 30),
      },
