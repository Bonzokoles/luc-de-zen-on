...
export const POST: APIRoute = async ({ request }) => {
  try {
    const { url, checks = [] } = (await request.json()) as any;
    ...
    if (!response.ok) {
      return createErrorResponse(
        `Nie można pobrać strony: ${response.status} ${response.statusText}`,
        400
      );
    }
...
async function analyzeSEO(html: string, url: string) {
  ...
  return {
    ...
    summary: {
      ...
      criticalIssues: checks.filter((c) => c.impact === "critical" && !c.passed)
        .length,
