...
// --- Pre-made Quizzes Data ---
const premadeQuizzes = new Map([
  [
    "iq-logic-1",
    {
      id: "iq-logic-1",
      ...
      questions: [
        {
          question:
            "Która liczba nie pasuje do reszty: 2, 3, 5, 7, 11, 13, 15?",
          answers: ["3", "7", "11", "15"],
          correctAnswer: "15",
        ...
        {
          question:
            "Jeśli jutro jest pojutrze, to jaki dzień był wczoraj, jeśli dzisiaj jest niedziela?",
          answers: ["Piątek", "Sobota", "Czwartek", "Środa"],
          correctAnswer: "Piątek",
          explanation:
            "Jeśli 'jutro jest pojutrze', to 'dzisiaj' jest wczoraj. Skoro dzisiaj jest niedziela, to 'wczoraj' była sobota. A dniem przed sobotą jest piątek.",
        },
        {
          ...
          correctAnswer: "36",
          explanation:
            "Sekwencja to kwadraty kolejnych liczb naturalnych (1^2, 2^2, 3^2, itd.).",
        },
      ],
    },
  ],
      ...
      questions: [
        {
          ...
          explanation:
            "Chociaż Sydney jest największym miastem, stolicą Australii jest Canberra.",
        },
        {
          ...
          correctAnswer: "8",
          explanation:
            "Od 2006 roku, kiedy Pluton został przeklasyfikowany na planetę karłowatą, w Układzie Słonecznym jest 8 planet.",
        },
      ],
    },
  ],
      ...
      questions: [
        {
          ...
          explanation:
            "Ridley Scott jest reżyserem oryginalnego 'Łowcy androidów' (Blade Runner).",
        },
      ],
    },
  ],
...
// --- GET Handler: For fetching pre-made quizzes ---
export const GET: APIRoute = async ({ url }) => {
  const list = url.searchParams.get("list");
  const id = url.searchParams.get("id");

  if (list) {
    const quizList = Array.from(premadeQuizzes.values()).map((q) => ({
      id: q.id,
      title: q.title,
      topic: q.topic,
      difficulty: q.difficulty,
    }));
    return new Response(JSON.stringify({ success: true, quizzes: quizList }), {
      status: 200,
    });
  }
  ...
  if (id) {
    ...
    if (quiz) {
      return new Response(JSON.stringify({ success: true, quiz: quiz }), {
        status: 200,
      });
    } else {
      return new Response(
  ...
  return new Response(
    JSON.stringify({
      success: false,
      error: "Invalid request. Use ?list=true or ?id=<quiz_id>.",
    }),
    { status: 400 }
  );
};

// --- POST Handler: For generating quizzes with AI ---
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const systemPrompt = `You are an expert at creating educational quizzes. Create a quiz on the topic of "${topic}" at the "${
      difficultyLevels[difficulty as keyof typeof difficultyLevels]
    }" level. You must generate exactly ${questionCount} questions. You must respond with a valid JSON object only.`;
    ...
    try {
      ...
      console.error(
        "Failed to parse AI response for quiz:",
        aiResponse.response
      );
