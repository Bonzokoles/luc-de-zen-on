...
const TEST_CONFIG: TestConfig = {
  ...
  workerUrl:
    ...
    "https://luc-de-zen-on.stolarnia-ams.workers.dev",
  timeout: 10000,
  retries: 3,
};
...
describe("MyBonzo - Testy Konfiguracji Systemu", () => {
  test("Zmienne środowiskowe - krytyczne", () => {
    ...
    const criticalEnvs = ["PUBLIC_WORKER_BASE_URL", "OPENAI_API_KEY"];
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      console.log(
        `ℹ️ Development mode - pomijam weryfikację ${missing.length} zmiennych`
      );
    }
  });
...
describe("MyBonzo - Testy API Endpoints", () => {
    ...
    async () => {
      await withRetry(async () => {
        ...
        const response = await fetch(
          `${TEST_CONFIG.baseUrl}/api/system/validate`,
          {
            method: "GET",
  ...
  test(
    ...
    async () => {
      await withRetry(async () => {
        ...
        const response = await fetch(
          `${TEST_CONFIG.baseUrl}/api/system/validate?full=true`,
          {
            method: "GET",
            signal: AbortSignal.timeout(TEST_CONFIG.timeout),
          }
        ...
        if (response.ok) {
          ...
          console.log(
            `✅ Pełna diagnostyka: ${
              diagnostics.summary?.overallStatus || "unknown"
            }`
          );
        }
      });
  ...
  test("Music library API", async () => {
    await withRetry(async () => {
      ...
      try {
        const response = await fetch(
          `${TEST_CONFIG.baseUrl}/api/music/library`,
          {
            method: "GET",
        ...
        console.warn(
          `⚠️ Music Library API niedostępny: ${
...
describe("MyBonzo - Testy Cloudflare Workers", () => {
  test("Worker health endpoint", async () => {
    ...
    await withRetry(async () => {
      console.log(
        `🔍 Testowanie Worker health: ${TEST_CONFIG.workerUrl}/health`
      );
      ...
      try {
        ...
        console.warn(
          `⚠️ Worker health niedostępny: ${(error as any).message || "Unknown"}`
        );
...
describe("MyBonzo - Testy Połączeń Zewnętrznych", () => {
  ...
  test("Cloudflare API connectivity", async () => {
    ...
    await withRetry(async () => {
      ...
      try {
        ...
        console.warn(
          `⚠️ Błąd połączenia z Cloudflare: ${
            (error as any).message || "Unknown"
          }`
        );
  ...
  test(
    ...
    async () => {
      ...
      console.log(
        `✅ Połączenie internetowe OK (${successCount}/${testUrls.length} serwisów dostępnych)`
      );
    },
    { timeout: 15000 }
  );
...
describe("MyBonzo - Testy Wydajności", () => {
  test("API response times", async () => {
    ...
    const endpoints = ["/api/system/validate"];
...
beforeAll(async () => {
  ...
  console.log(
    `☁️ Worker URL: ${TEST_CONFIG.workerUrl || "nie skonfigurowany"}`
  );
