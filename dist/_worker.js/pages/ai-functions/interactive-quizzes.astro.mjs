...
function InteractiveQuizWidget($$payload, $$props) {
	...
	$$payload.out += `<div class="quiz-widget-container svelte-1s2xbne"><h2 class="widget-title svelte-1s2xbne">🧠 Interaktywny Quiz AI</h2> `;

	{
		...
		$$payload.out += `<div class="quiz-setup svelte-1s2xbne"><div class="setup-section svelte-1s2xbne"><h3 class="setup-title svelte-1s2xbne">📖 Wybierz temat</h3> <div class="topics-grid svelte-1s2xbne"><!--[-->`;

		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			let topic = each_array[$$index];

			$$payload.out += `<button${attr('class', to_class('topic-card ' + (selectedTopic === topic.id ? 'selected' : ''), 'svelte-1s2xbne'))}><div class="topic-name svelte-1s2xbne">${escape_html(topic.name)}</div> <div class="topic-description svelte-1s2xbne">${escape_html(topic.description)}</div></button>`;
		}

		$$payload.out += `<!--]--></div></div> <div class="setup-section svelte-1s2xbne"><h3 class="setup-title svelte-1s2xbne">⚡ Poziom trudności</h3> <div class="difficulty-grid svelte-1s2xbne"><!--[-->`;

		for (let $$index_1 = 0,
			$$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
			let level = each_array_1[$$index_1];

			$$payload.out += `<button${attr('class', to_class('difficulty-card ' + (difficulty === level.id ? 'selected' : ''), 'svelte-1s2xbne'))}><div class="difficulty-name svelte-1s2xbne">${escape_html(level.name)}</div> <div class="difficulty-description svelte-1s2xbne">${escape_html(level.description)}</div></button>`;
		}

		$$payload.out += `<!--]--></div></div> <button class="start-quiz-btn svelte-1s2xbne"${attr('disabled', true, true)}>🚀 Rozpocznij Quiz</button></div>`;
	}
