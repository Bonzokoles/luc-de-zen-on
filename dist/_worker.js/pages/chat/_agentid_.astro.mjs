...
import { p as push, l as ensure_array_like, j as escape_html, i as attr, t as to_class, w as bind_props, k as pop, e as createAstro, c as createComponent, r as renderComponent, a as renderTemplate, m as maybeRenderHead } from '../../chunks/vendor_QZhDtzeH.mjs';
...
function DynamicChat($$payload, $$props) {
	...
	$$payload.out += `<!--]--></div> <div class="chat-input border-t border-edge p-4"><div class="flex gap-3"><textarea rows="2" placeholder="Napisz swoją wiadomość..." class="flex-1 p-3 border border-edge rounded-lg text-primary-foreground placeholder-gray-400 focus:border-cyan-400 focus:outline-none resize-none" style="background: rgba(0, 0, 0, 0.5);"${attr('disabled', isLoading, true)}>`;
	...
	$$payload.out += `</textarea> <button class="bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-200 disabled:opacity-50"${attr('disabled', isLoading, true)}>Wyślij</button></div></div></div>`;
