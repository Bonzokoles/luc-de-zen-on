...
import { p as push, l as ensure_array_like, i as attr, j as escape_html, t as to_class, F as stringify, k as pop } from './vendor_QZhDtzeH.mjs';
/* empty css                                      */

function WorkersStatusDashboard($$payload, $$props) {
	push();
	...
	$$payload.out += `<div class="w-full bg-[#111111] border border-[#333333] glass-effect p-6 svelte-18wsoh5"><div class="flex items-center justify-between mb-6"><div><h2 class="text-2xl font-bold text-[#00ffff] font-['Neuropol'] uppercase tracking-wider">🔧 Status Workerów</h2> <p class="text-[#a0a0a0] font-['Kenyan_Coffee'] mt-1">Zaawansowany monitoring systemu AI funkcji w czasie rzeczywistym</p></div> <div class="flex items-center gap-3"><button${attr('class', to_class('px-3 py-1 text-xs font-[\'Neuropol\'] uppercase border transition-all duration-300', 'svelte-18wsoh5', {
		'auto-refresh-active': autoRefresh,
		'auto-refresh-inactive': false
	}))}>Auto: ${escape_html("ON" )}</button> `;

	if (metrics.lastUpdate) {
		$$payload.out += '<!--[-->';
		$$payload.out += `<span class="text-xs text-[#666666] font-['Kenyan_Coffee']">Last: ${escape_html(formatTime(metrics.lastUpdate))}</span>`;
	} else {
		$$payload.out += '<!--[!-->';
	}
	...
	if (metrics.systemHealth !== undefined) {
		...
		$$payload.out += `<div class="mb-6"><div class="flex justify-between text-sm mb-2"><span class="text-[#a0a0a0] font-['Kenyan_Coffee']">Ogólny Stan Systemu</span> <span class="text-[#00ffff] font-['Neuropol']">${escape_html(Math.round(metrics.systemHealth))}%</span></div> <div class="w-full bg-[#333333] h-2 border border-[#555555]"><div${attr('class', to_class('h-full transition-all duration-500', null, {
	...
	if (workers.length > 0) {
		...
		$$payload.out += `<span class="text-[#e0e0e0] font-['Kenyan_Coffee']"><span class="text-green-400">${escape_html(metrics.onlineWorkers)}</span> / <span class="text-[#00ffff]">${escape_html(metrics.totalWorkers)}</span> workerów
        online `;

		if (metrics.systemHealth) {
			$$payload.out += '<!--[-->';
			$$payload.out += `- System Health: <span class="text-[#00ffff]">${escape_html(Math.round(metrics.systemHealth))}%</span>`;
		} else {
			$$payload.out += '<!--[!-->';
		}
		...
		$$payload.out += '<!--[!-->';
		$$payload.out += `<span class="text-[#a0a0a0] font-['Kenyan_Coffee']">Brak danych o workerach</span>`;
	}

	$$payload.out += `<!--]--></div> <div class="overflow-x-auto border border-[#333333]"><table class="min-w-full bg-[#0a0a0a]"><thead><tr class="border-b border-[#333333]"><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">Nazwa Workera</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">Status</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">CPU</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">RAM</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">Req/min</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">Response</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">Uptime</th><th class="px-4 py-3 text-left text-[#00ffff] font-['Neuropol'] uppercase text-sm">Last Check</th></tr></thead><tbody><!--[-->`;

	for (let i = 0, $$length = each_array.length; i < $$length; i++) {
		let worker = each_array[i];

		$$payload.out += `<tr class="border-b border-[#333333]/50 hover:bg-[#111111] transition-colors duration-200"><td class="px-4 py-3"><div class="text-[#e0e0e0] font-['Kenyan_Coffee'] font-medium">${escape_html(worker.name)}</div> <div class="text-xs text-[#666666] font-['Kenyan_Coffee']">${escape_html(worker.endpoint)}</div></td><td class="px-4 py-3"><span${attr('class', to_class('px-2 py-1 text-xs font-[\'Neuropol\'] uppercase border ' + getStatusBg(worker.status) + ' ' + getStatusColor(worker.status), 'svelte-18wsoh5'))}>${escape_html(worker.status)}</span></td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.cpu !== null ? worker.cpu + "%" : "-")}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.ram !== null ? worker.ram + "%" : "-")}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.requests || 0)}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.responseMs !== null ? worker.responseMs + "ms" : "-")}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(formatUptime(worker.uptime))}</td><td class="px-4 py-3 text-[#a0a0a0] font-['Kenyan_Coffee'] text-sm">${escape_html(worker.lastCheck ? formatTime(worker.lastCheck) : "-")}</td></tr>`;
	}

	$$payload.out += `<!--]--></tbody></table></div> <div class="mt-6 flex flex-col gap-3"><button${attr('disabled', loading, true)} class="w-full p-3 bg-[#00ffff]/10 border border-[#00ffff]/30 text-[#00ffff] hover:bg-[#00ffff]/20 transition-all duration-300 font-['Neuropol'] uppercase tracking-wide disabled:opacity-50">${escape_html("Sprawdź Wszystkie Workery")}</button> <button class="w-full p-3 bg-[#00ffff]/10 border border-[#00ffff]/30 text-[#00ffff] hover:bg-[#00ffff]/20 transition-all duration-300 font-['Neuropol'] uppercase tracking-wide">${escape_html("Pokaż")} Szczegółowy Monitor</button></div> `;
