...
import { j as attr_class, k as escape_html, C as attr_style, n as attr, i as ensure_array_like, l as stringify } from './vendor_BHZTJLV0.mjs';
/* empty css                                      */
...
function WorkersStatusDashboard($$payload, $$props) {
	$$payload.component(($$payload) => {
		...
		if (metrics.lastUpdate) {
			$$payload.push('<!--[-->');
			$$payload.push(`<span class="text-xs text-[#666666] font-['Kenyan_Coffee']">Last: ${escape_html(formatTime(metrics.lastUpdate))}</span>`);
		} else {
			$$payload.push('<!--[!-->');
		}
...

		{
			...
			if (workers.length > 0) {
				...
				$$payload.push(`<span class="text-[#e0e0e0] font-['Kenyan_Coffee']"><span class="text-green-400">${escape_html(metrics.onlineWorkers)}</span> / <span class="text-[#00ffff]">${escape_html(metrics.totalWorkers)}</span> workerów
        online `);

				if (metrics.systemHealth) {
					$$payload.push('<!--[-->');
					$$payload.push(`- System Health: <span class="text-[#00ffff]">${escape_html(Math.round(metrics.systemHealth))}%</span>`);
				} else {
					$$payload.push('<!--[!-->');
				}
		...
		for (let i = 0, $$length = each_array.length; i < $$length; i++) {
			...
			$$payload.push(`<tr class="border-b border-[#333333]/50 hover:bg-[#111111] transition-colors duration-200"><td class="px-4 py-3"><div class="text-[#e0e0e0] font-['Kenyan_Coffee'] font-medium">${escape_html(worker.name)}</div> <div class="text-xs text-[#666666] font-['Kenyan_Coffee']">${escape_html(worker.endpoint)}</div></td><td class="px-4 py-3"><span${attr_class(`px-2 py-1 text-xs font-['Neuropol'] uppercase border ${stringify(getStatusBg(worker.status))} ${stringify(getStatusColor(worker.status))}`, 'svelte-9s5lrx')}>${escape_html(worker.status)}</span></td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.cpu !== null ? worker.cpu + "%" : "-")}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.ram !== null ? worker.ram + "%" : "-")}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.requests || 0)}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(worker.responseMs !== null ? worker.responseMs + "ms" : "-")}</td><td class="px-4 py-3 text-[#e0e0e0] font-['Kenyan_Coffee']">${escape_html(formatUptime(worker.uptime))}</td><td class="px-4 py-3 text-[#a0a0a0] font-['Kenyan_Coffee'] text-sm">${escape_html(worker.lastCheck ? formatTime(worker.lastCheck) : "-")}</td></tr>`);
		}

		$$payload.push(`<!--]--></tbody></table></div> <div class="mt-6 flex flex-col gap-3"><button${attr('disabled', loading, true)} class="w-full p-3 bg-[#00ffff]/10 border border-[#00ffff]/30 text-[#00ffff] hover:bg-[#00ffff]/20 transition-all duration-300 font-['Neuropol'] uppercase tracking-wide disabled:opacity-50">${escape_html("Sprawdź Wszystkie Workery")}</button> <button class="w-full p-3 bg-[#00ffff]/10 border border-[#00ffff]/30 text-[#00ffff] hover:bg-[#00ffff]/20 transition-all duration-300 font-['Neuropol'] uppercase tracking-wide">${escape_html("Pokaż")} Szczegółowy Monitor</button></div> `);
