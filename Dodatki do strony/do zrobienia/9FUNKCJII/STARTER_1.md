Oto rozszerzony skrypt Node.js do automatycznego tworzenia struktury folderów workspace `T:\MY_LUC_ZEN_ON` wraz z kopiowaniem przykładowych szablonów plików (frontend, backend, workflow i dokumentacja) do odpowiednich katalogów dla szybkiego startu projektu mybonzo:

***

# Skrypt automatycznego tworzenia struktury i kopiowania szablonów
...
1. Zapisz powyższy skrypt jako `createProjectStructureWithTemplates.js` na komputerze.
2. Upewnij się, że masz zainstalowanego Node.js wersja 14+.
3. Uruchom w terminalu:

```
node createProjectStructureWithTemplates.js
```

4. Skrypt utworzy całą strukturę folderów wraz z przykładowymi plikami szablonowymi i plikiem `rules.txt`.
5. Po utworzeniu możesz rozszerzać foldery i pliki o kolejne komponenty według potrzeb.

***

Jeśli chcesz, mogę przygotować podobne pliki szablonowe dla kolejnych funkcjonalności lub dopisać automatyczny skrypt deploymentu.